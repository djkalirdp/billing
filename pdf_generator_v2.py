"""
================================================================================
  pdf_generator_v2.py  —  Preview 2 (11 Fixed Rows Per Page)
...
def create_invoice_pdf(invoice_data, items, settings,
    ...
    for copy_idx, copy_name in enumerate(copies):
        ...
        # ── V2 Seller Header — MSP Traders style ─────────────────
        # GSTIN top-left | "Original Copy" top-right (single line)
        # Then: TAX INVOICE centered
        # Then: COMPANY NAME large bold centered
        # Then: Address, Tel, Email centered
        # All without box — just text, then a horizontal line
        # ─────────────────────────────────────────────────────────

        _cell_w  = 9.5 * W_SCALE   # used later for invoice/buyer boxes

        # Styles for centered header
        _s_gstin = ParagraphStyle('v2gstin', fontName=FONT_REG,  fontSize=round(8*FS),
                                  leading=10, alignment=TA_LEFT)
        _s_copy  = ParagraphStyle('v2copy',  fontName=FONT_REG,  fontSize=round(8*FS),
                                  leading=10, alignment=TA_RIGHT)
        _s_title = ParagraphStyle('v2title', fontName=FONT_BOLD, fontSize=round(9*FS),
                                  leading=12, alignment=TA_CENTER, spaceAfter=1)
        _s_name  = ParagraphStyle('v2name',  fontName=FONT_BOLD, fontSize=round(16*FS),
                                  leading=20, alignment=TA_CENTER, spaceAfter=2)
        _s_addr  = ParagraphStyle('v2addr',  fontName=FONT_REG,  fontSize=round(8.5*FS),
                                  leading=11, alignment=TA_CENTER)

        # Total usable width = _cell_w * 2 cm
        _full_w = _cell_w * 2

        # Row 1: GSTIN left | Original Copy right
        gstin_str = c_info.get('gstin', '')
        copy_label_str = copy_name if not is_proforma else 'QUOTATION'
        t_toprow = Table(
            [[Paragraph(f"GSTIN : {gstin_str}", _s_gstin),
              Paragraph(copy_label_str, _s_copy)]],
            colWidths=[_full_w * 0.6 * cm, _full_w * 0.4 * cm]
        )
        t_toprow.setStyle(TableStyle([
            ('TOPPADDING',    (0,0), (-1,-1), 2),
            ('BOTTOMPADDING', (0,0), (-1,-1), 2),
            ('LEFTPADDING',   (0,0), (0,0),   2),
            ('RIGHTPADDING',  (1,0), (1,0),   2),
        ]))
        elements.append(t_toprow)

        # Row 2: TAX INVOICE (centered)
        elements.append(Paragraph(doc_title, _s_title))
        ...
        if logo_img:
            ...
            elements.append(Paragraph(f"<b>{company_name}</b>", _s_name))

        # Row 4: Address line1 + line2 centered
        addr1 = c_info.get('address_line1', '')
        addr2 = c_info.get('address_line2', '')
        full_addr = ', '.join(filter(None, [addr1, addr2]))
        if full_addr:
            elements.append(Paragraph(full_addr, _s_addr))

        # Row 5: Tel + Email centered
        ...
        if contact_parts:
            elements.append(Paragraph('  '.join(contact_parts), _s_addr))

        elements.append(Spacer(1, 3))

        # Horizontal divider line below seller header
        from reportlab.platypus import HRFlowable
        elements.append(HRFlowable(width="100%", thickness=0.8, color=colors.black, spaceAfter=3))

        _logo_cw = 2.2 * W_SCALE   # kept for compatibility (unused in v2 header)
        _text_cw = _cell_w - _logo_cw
        ...
        # ── V2 Header Layout: Invoice details | Buyer details ───
        # Seller is already rendered above as centered paragraphs
        # Now: left = Invoice No/Date/Mode/Dispatch | right = Buyer info
        t_head = Table(
            [[t_inv, t_buyer]],
        ...
        # Empty row template
        def _empty_row():
            return [Paragraph('', S['item_center'])] * 9

        # Split into pages of ROWS_PER_PAGE
        # Last page gets totals row — so last page has ROWS_PER_PAGE - 1 data rows + 1 total
        total_items = len(all_item_rows)
        if total_items == 0:
            pages_items = [[]]
        elif total_items <= ROWS_PER_PAGE - 1:
            # All fits on one page
            pages_items = [all_item_rows]
        else:
            # First pages: full ROWS_PER_PAGE data rows each
            pages_items = []
            i = 0
            while i < total_items:
                if i + ROWS_PER_PAGE - 1 >= total_items:
                    # Last page — remaining items
                    pages_items.append(all_item_rows[i:])
                    break
                else:
                    pages_items.append(all_item_rows[i:i + ROWS_PER_PAGE])
                    i += ROWS_PER_PAGE

        # Totals row
        totals_row = [
            Paragraph('', S['item_center']),
        ...
        # Fixed row height — same for filled and empty rows
        # Header row taller, data rows all equal height
        ROW_H     = 22   # pt — data row height (filled & empty both)
        HEADER_H  = 20   # pt — header row height

        # Build tables for each page
        for pg_idx, page_rows in enumerate(pages_items):
            is_last_page = (pg_idx == len(pages_items) - 1)
            ...
            # Pad with empty rows
            if is_last_page:
                # Last page: pad to ROWS_PER_PAGE - 1 then add totals
                while len(table_data) - 1 < ROWS_PER_PAGE - 1:
                    table_data.append(_empty_row())
                table_data.append(totals_row)
            else:
                # Middle pages: pad to ROWS_PER_PAGE (no totals row)
                while len(table_data) - 1 < ROWS_PER_PAGE:
                    table_data.append(_empty_row())

            # rowHeights: header + all data rows same fixed height
            # Scale for A5
            _rh = ROW_H    * W_SCALE
            _hh = HEADER_H * W_SCALE
            row_heights = [_hh] + [_rh] * (len(table_data) - 1)
            ...
            if is_last_page:
                ...
                ts.add('LINEABOVE', (0,-1), (-1,-1), 0.8, colors.black)
            t_items.setStyle(ts)
            elements.append(t_items)

            # If not last page, add page break + header again
            if not is_last_page:
                elements.append(PageBreak())
                # Re-add invoice header for continuation page
                elements.append(Paragraph(
                    f"<b>{doc_title}</b> (Continued) — {invoice_data.get('invoice_no', '')}",
                    ParagraphStyle('cont', fontName='Helvetica-Bold', fontSize=10,
