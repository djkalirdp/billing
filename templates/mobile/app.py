...
def billing():
    ...
    existing_buyers = {b['name']: b for b in db.get_all('buyers')}
    if buyer_name in existing_buyers:
        buyer_id = existing_buyers[buyer_name]['id']
    else:
        ...
        if not gstin or not address or not state:
            flash('For a new buyer, GSTIN, Address and State are required.', 'danger')
            return redirect(url_for('billing'))
    ...
    items_data = []
    products_dict = {p['id']: p for p in db.get_all('products')}
    ...
    for i in range(len(product_ids)):
        try:
            ...
            items_data.append({
                ...
                'discount_percent': float(discounts[i]) if i < len(discounts) else 0,
...
def view_invoice(invoice_id):
    ...
    return mrender('invoice_detail.html', invoice=inv, items=items, settings=load_settings())
...
def download_invoice_pdf(invoice_id):
    ...
    settings  = load_settings()
    pdf_bytes = pdf_generator.create_invoice_pdf(inv, items, settings, save_to_disk=True)
